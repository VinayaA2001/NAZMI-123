...
/* ---------- Page ---------- */
export default function WesternSlugPage() {
    ...
    (async () => {
      ...
      try {
        ...
        if (one.ok) {
          ...
          if (!/western/.test(cat)) throw new Error("Product is not a western item");
          setProduct(normalize(raw));
          return;
        }

        // Fallback: pull the western list and match locally
        const res = await fetch("/api/products?category=western", { cache: "no-store" });
        if (!res.ok) throw new Error("Failed to load western products");
  ...
  useEffect(() => {
    ...
    router.replace(`/western/${finalSlug}?${q.toString()}`);
  ...
  const pickColor = (c: string) => {
    ...
    router.replace(`/western/${product.slug || makeSlug(product)}?${q.toString()}`);
  };
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => router.push("/western")} className="inline-flex items-center gap-2 px-4 py-2 border rounded">
          <ArrowLeft className="w-4 h-4" /> Back to Western
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-6">
        <button onClick={() => router.push("/western")} className="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900">
          <ArrowLeft className="w-4 h-4" /> Back to Western
        </button>
      </div>
